...
cnty_clus_offices$crmnlty <- relevel(cnty_clus_offices$crmnlty, ref = "3 Other Immigration Violator")

cnty_clus_offices$led_vnt <- factor(cnty_clus_offices$led_vnt)
cnty_clus_offices$led_vnt <- relevel(cnty_clus_offices$led_vnt, ref = "Not applicable")

cnty_clus_offices$clstr_f <- relevel(cnty_clus_offices$clstr_f, ref = "43")
...
```{r}
cnty_clus_offices <- cnty_clus_offices |>
  mutate(prog_287g = ifelse(evnt_ty == '287(g) Program Activities', 1, 0)) 
...
cluster_reg <- glm(deportd ~ crmnlty + clstr_f + crmnlty:clstr_f+ gender + age + region + avg_p_b + avg_p_a + avg_p_h +  avg_p_n  + avg_pop_sqm, 
               family = binomial,
               data = cnty_clus_offices)
...
```{r}
cluster_reg <- readRDS("models/cluster_reg1.rds")
```
...
#### Cohen's D
```{r}
clstr_crim_d <- oddsratio_to_d(exp(coef(cluster_reg)))
```

Interpretation: d = 0.2 is small, d = 0.5 is medium, and d = 0.8 is large
```{r}
clstr_crim_d_df <- stack(clstr_crim_d) |>
...
```{r}
freq_table <- table(cnty_clus_offices$crmnlty)
prop.table(freq_table)
```


```{r}
table(cnty_clus_offices$region)
```
...
Check the reference cluster: 43
```{r}
all_clusters <- unique(cnty_clus_offices$clstr_f)

coef_names <- names(coef(cluster_reg))  

present <- gsub("clstr_f", "", coef_names[grepl("^clstr_f", coef_names)])
present <- as.numeric(present)

setdiff(all_clusters, present)
```
...
coefs <- summary(cluster_reg)$coefficients
```


```{r}
# Extract main effect of convicted criminal (same for all clusters)
clstr_fixed_crim <- coefs["crmnlty1 Convicted Criminal", "Estimate"]

# Extract interaction terms (vary by cluster)
clstr_int_crim <- coefs[grep("^crmnlty1 Convicted Criminal:clstr_f", rownames(coefs)), , drop = FALSE]
...
interaction_df <- data.frame(
  clstr_f = sub("crmnlty1 Convicted Criminal:clstr_f", "", rownames(clstr_int_crim)),
...
all_clusters <- unique(cluster_office_df$clstr_f)  # Get all cluster IDs from your data
interaction_df <- data.frame(clstr_f = all_clusters) |>
...
cluster_office_df <- cnty_clus_offices |>
  group_by(office) |>
  select(c(office, clstr_f, geometry, aor)) |>
  slice(1) |>
  ungroup()
```
...
```{r}
table(cnty_clus_offices$led_vnt)
```

Most regions are signiicantly different from each other
```{r}
region_means <- emmeans(cluster_reg, ~ region)
...
cnty_clus_offices |>
  group_by(aor)|>
  summarize(prop_noncrim = mean(crmnlty == "3 Other Immigration Violator")) |>
...
aor_reg <- glm(deportd ~ crmnlty + aor + crmnlty:aor + gender + age + region + avg_p_b + avg_p_a + avg_p_h + avg_p_n+  avg_pop_sqm, 
               family = binomial,
               data = cnty_clus_offices)
...
aor_fixed_crim <- coefs["crmnlty1 Convicted Criminal", "Estimate"]

# Extract interaction terms (vary by AOR)
aor_int_crim <- coefs[grep("^crmnlty1 Convicted Criminal:aor", rownames(coefs)), , drop = FALSE]
...
aor_interaction_df <- data.frame(
  aor = sub("crmnlty1 Convicted Criminal:aor", "", rownames(aor_int_crim)),
...
```{r}
cluster_age <- glm(deportd ~ crmnlty+ clstr_f +age:clstr_f+ gender + age + region + avg_p_b + avg_p_a + avg_p_h +  avg_p_n  + avg_pop_sqm, 
               family = binomial,
               data = cnty_clus_offices)
...
# Create interaction dataframe - strip clstr_f prefix and :age suffix
age_interaction_df <- data.frame(
  clstr_f = sub("clstr_f", "", sub(":age$", "", rownames(cluster_int_age))),
  cluster_int_age = cluster_int_age[, "Estimate"]
)

# Get all unique clusters
all_clusters <- unique(cluster_office_df$clstr_f)

# Add fixed effect and handle missing interactions
age_interaction_df <- data.frame(clstr_f = all_clusters) |>
  left_join(age_interaction_df, by = "clstr_f") |>
  mutate(
...
```{r}
cluster_hisp <- glm(deportd ~ crmnlty +clstr_f + avg_p_h:clstr_f+ gender + age + region + avg_p_b + avg_p_a + avg_p_h +  avg_p_n  + avg_pop_sqm, 
               family = binomial,
               data = cnty_clus_offices)
...
# Extract main effect of age
cluster_fixed_hisp <- coefs["avg_p_h", "Estimate"]

# Extract interaction terms
cluster_int_hisp <- coefs[grep(":avg_p_h$", rownames(coefs)), , drop = FALSE]
...
hisp_interaction_df <- data.frame(
  clstr_f = sub("clstr_f", "", sub(":avg_p_h$", "", rownames(cluster_int_hisp))),
...
# Add fixed effect and handle missing interactions
hisp_interaction_df <- data.frame(clstr_f = all_clusters) |>
  left_join(hisp_interaction_df, by = "clstr_f") |>
  mutate(
...
WITHOUT REGION
```{r}
cluster_hisp2 <- glm(deportd ~ crmnlty +clstr_f + avg_p_h:clstr_f+ gender + age + avg_p_b + avg_p_a + avg_p_h +  avg_p_n  + avg_pop_sqm, 
               family = binomial,
               data = cnty_clus_offices)
...
# Extract main effect of age
cluster_fixed_hisp2 <- coefs["avg_p_h", "Estimate"]

# Extract interaction terms
cluster_int_hisp2 <- coefs[grep(":avg_p_h$", rownames(coefs)), , drop = FALSE]
...
hisp2_interaction_df <- data.frame(
  clstr_f = sub("clstr_f", "", sub(":avg_p_h$", "", rownames(cluster_int_hisp2))),
...
# Add fixed effect and handle missing interactions
hisp2_interaction_df <- data.frame(clstr_f = all_clusters) |>
  left_join(hisp2_interaction_df, by = "clstr_f") |>
  mutate(
...
```{r}
cluster_asian <- glm(deportd ~ crmnlty + clstr_f +avg_p_a:clstr_f+ gender + age + region + avg_p_b + avg_p_a + avg_p_h +  avg_p_n  + avg_pop_sqm, 
               family = binomial,
               data = cnty_clus_offices)
...
# Extract main effect of age
cluster_fixed_asi <- coefs["avg_p_a", "Estimate"]

# Extract interaction terms
cluster_int_asi <- coefs[grep(":avg_p_a$", rownames(coefs)), , drop = FALSE]
...
asi_interaction_df <- data.frame(
  clstr_f = sub("clstr_f", "", sub(":avg_p_a$", "", rownames(cluster_int_asi))),
  cluster_int_asi = cluster_int_asi[, "Estimate"]
)


# Add fixed effect and handle missing interactions
asi_interaction_df <- data.frame(clstr_f = all_clusters) |>
  left_join(asi_interaction_df, by = "clstr_f") |>
  mutate(
...
```
```{r}
cluster_latin_cit<- glm(deportd ~ crmnlty + clstr_f +latin_cit:clstr_f+ gender + age + latin_cit + avg_p_b + avg_p_a + avg_p_h +  avg_p_n  + avg_pop_sqm, 
               family = binomial,
               data = cnty_clus_offices)
...
lat_mexico_df <- data.frame(
  clstr_f = sub("clstr_f", "", sub(":latin_citMexico$", "", rownames(cluster_int_mexico))),
  cluster_int_mexico = cluster_int_mexico[, "Estimate"]
)

lat_samerica_df <- data.frame(
  clstr_f = sub("clstr_f", "", sub(":latin_citSouth America$", "", rownames(cluster_int_samerica))),
  cluster_int_samerica = cluster_int_samerica[, "Estimate"]
)

lat_central_df <- data.frame(
  clstr_f = sub("clstr_f", "", sub(":latin_citCentral America/Carribbean$", "", rownames(cluster_int_central))),
...
lat_interaction_df <- data.frame(clstr_f = all_clusters) |>
  left_join(lat_mexico_df, by = "clstr_f") |>
  left_join(lat_samerica_df, by = "clstr_f") |>
  left_join(lat_central_df, by = "clstr_f") |>
  mutate(
...
# Join to main dataframe
cluster_office_df <- cluster_office_df |>
  left_join(lat_interaction_df, by = "clstr_f")
```
...
## Make dataframe of effect sizes
```{r}
cohens_d_dfs <- list(clstr_crim_d_df, aor_crim_d_df, clstr_age_d_df, clstr_hisp_d_df, clstr_asi_d_df, cluster_lat_d_df)
```
One row = one cluster
Columns: cohen's D size for each of the terms of interest (fixed, then all of the interactions)

For clstr_crim_d_df, filter to the fixed effects and interactions

For the rest of them filter to only the interactions

Create a column of cluster name

From criminality model df
```{r}
clstr_crim_filt <- clstr_crim_d_df |>
  filter(grepl("^crmnlty1 Convicted Criminal:", coef) |grepl("^clstr_f", coef)) |>
  mutate(cluster = sub("^.*clstr_f([0-9]+).*", "\\1", coef),
         coef_type = case_when(
      grepl("^clstr_f", coef) ~ "fixed",
      grepl("^crmnlty1 Convicted Criminal:", coef) ~ "interaction_crim"
    ))

head(clstr_crim_filt)
```

```{r}
clstr_crim_filt_wider <- clstr_crim_filt|>
  select(cluster, size_cat, coef_type) |>
  pivot_wider(
    names_from = coef_type,
    values_from = size_cat,
    names_prefix = "size_cat_"
  )

clstr_crim_filt_wider 
```


For the rest of them
```{r}
process_cluster_interactions <- function(df, interaction_suffix) {
  df |>
    filter(grepl(paste0(":", interaction_suffix, "$"), coef)) |>
    mutate(
      cluster = sub("^.*clstr_f([0-9]+).*", "\\1", coef),
      coef_type = paste0("interaction_", interaction_suffix)
    ) |>
    select(cluster, size_cat, coef_type) |>
    pivot_wider(
      names_from = coef_type,
...
clstr_age_filt_wider <- process_cluster_interactions(clstr_age_d_df, "age")
clstr_hisp_filt_wider <- process_cluster_interactions(clstr_hisp_d_df, "avg_p_h")
clstr_asi_filt_wider <- process_cluster_interactions(clstr_asi_d_df, "avg_p_a")
clstr_lat_filt_wider <- process_cluster_interactions(cluster_lat_d_df, "latin_am")
```

Now time to join them all together
```{r}
effect_sizes <- clstr_crim_filt_wider |>
  left_join(clstr_age_filt_wider, by="cluster") |>
  left_join(clstr_hisp_filt_wider, by="cluster") |>
  left_join(clstr_asi_filt_wider, by="cluster") |>
  left_join(clstr_lat_filt_wider, by="cluster") 
...
Save as csv
```{r}
write_csv(effect_sizes, "data/effect_size_clusters.csv")
```
...
glmer_reg_nested1 <- glmer(
  deportd ~ crmnlty + gender + scale(age) + region + scale(avg_p_b) +
             scale(avg_p_a) + scale(avg_p_h) + scale(avg_p_n) + scale(avg_pop_sqm) +
             (1 | aor/clstr_f),
  family = binomial,
  control = glmerControl(optimizer = "nloptwrap"),
  data = cnty_clus_offices
