...
```{r}
cluster_interaction_coefs_3 <- coef(cluster_reg)[grep("crmnlty1 Convicted Criminal:", names(coef(cluster_reg)))]

clstr_fixed_effects_3 <-  coef(cluster_reg)[grep("^clstr_f", names(coef(cluster_reg)))]
```
...
interaction_df <- data.frame(
  ...
  clstr_crim3_fixed = as.numeric(clstr_fixed_effects_3)
) |>
...
```{r}
aor_interaction_coefs_3 <- coef(aor_reg)[grep("crmnlty1 Convicted Criminal:", names(coef(aor_reg)))]

aor_fixed_effects_3 <-  coef(aor_reg)[grep("^aor", names(coef(aor_reg)))]
```
...
interaction_df <- data.frame(
  term = names(aor_fixed_effects_3),
  aor_crim_3_int = as.numeric(aor_interaction_coefs_3),
  aor_crim_3_fixed = as.numeric(aor_fixed_effects_3)
) |>
...
clstr_fixed_age <-  coef(cluster_age)[grep("^clstr_f\\d+$", names(coef(cluster_age)))]
```
...
interaction_df <- data.frame(
  term = names(clstr_fixed_age),
  clstr_age_int = as.numeric(clstr_int_age),
  clstr_age_fixed = as.numeric(clstr_fixed_age)
) |>
  mutate(clstr_f = sub(".*clstr_f", "", term)) |>
  select(-term)
...
clstr_fixed_hisp <- coef(cluster_hisp)[grep("^clstr_f\\d+$", names(coef(cluster_hisp)))]
...
interaction_df <- data.frame(
  ...
  clstr_hisp_fixed = as.numeric(clstr_fixed_hisp)
) |>
  mutate(clstr_f = sub(".*clstr_f", "", term)) |>
  select(-term)
...
clstr_fixed_asian <- coef(cluster_asian)[grep("^clstr_f\\d+$", names(coef(cluster_asian)))]
...
interaction_df <- data.frame(
  term = names(clstr_fixed_asian),
  clstr_asi_int = as.numeric(clstr_int_asian),
  clstr_asi_fixed =  as.numeric(clstr_fixed_asian)
) |>
  mutate(clstr_f = sub(".*clstr_f", "", term)) |>
  select(-term)
...
cnty_clus_offices <- cnty_clus_offices |>
  mutate(latin_am = ifelse(region %in% latin_am_regions, 1, 0))
```

```{r}
table(cnty_clus_offices$latin_am)
```
```{r}
cluster_lat_am <- glm(deportd ~ crmnlty + clstr_f +latin_am:clstr_f+ gender + age + latin_am + avg_p_b + avg_p_a + avg_p_h +  avg_p_n  + avg_pop_sqm, 
               family = binomial,
               data = cnty_clus_offices)

saveRDS(cluster_lat_am, file="cluster_lat_am.rds")
```

```{r}
summary(cluster_lat_am)
```

#### Get coefficients for the interaction term for being from a Latin American country
```{r}
clstr_int_lat <- coef(cluster_lat_am)[grep(":latin_am$", names(coef(cluster_lat_am)))]
clstr_fixed_lat <- coef(cluster_lat_am)[grep("^clstr_f\\d+$", names(coef(cluster_lat_am)))]
...
interaction_df <- data.frame(
  term = names(clstr_fixed_lat),
  clstr_lat_int = as.numeric(clstr_int_lat),
  clstr_lat_fixed =  as.numeric(clstr_fixed_lat)
) |>
  mutate(clstr_f = sub(".*clstr_f", "", term)) |>
  select(-term)
...
Lots of large interactions
```{r}
cluster_lat_am_d_df <- stack(cluster_lat_am_d) |>
  mutate(size_cat = case_when(
    abs(values) < 0.2 ~ "trivial",
    abs(values) >= 0.2 & abs(values) < 0.5 ~ "small",
    abs(values) >= 0.5 & abs(values) < 0.8 ~ "medium",
...
cluster_office_df_transf <- cluster_office_df |>
  # fix naming error
  rename("clstr_crim1_int" = "clstr_crim3_int",
         "clstr_crim1_fixed" = "clstr_crim3_fixed") |>
  mutate(across(
    .cols = c(clstr_crim1_int, clstr_crim1_fixed, aor_crim_3_int, aor_crim_3_fixed, clstr_age_int, clstr_age_fixed, clstr_hisp_int,clstr_hisp_fixed, clstr_asi_int, clstr_asi_fixed, clstr_lat_int, clstr_lat_fixed),
    .fns = ~ (exp(.) - 1) * 100,
    .names = "{.col}_pctdiff"
  )) 
...
## Make dataframe of effect zies
```{r}
cohens_d_dfs <- c(clstr_crim_d_df, aor_crim_d_df, clstr_age_d_df, clstr_hisp_d_df, clstr_asi_d_df)
```
