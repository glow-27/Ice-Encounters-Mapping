...
freq_table <- table(cnty_clus_offices$program_type)
prop.table(table(cnty_clus_offices$program_type))
...
log_reg <-glm(deported ~ criminality + gender + age + latin_cit + program_type+ avg_p_blk + avg_p_asn  + avg_p_his + avg_p_nc + avg_pop_sqm, 
               family = binomial,
...
cluster_reg_crim <- glm(deported ~ criminality + cluster + criminality:cluster + gender + age + latin_cit, 
               family = binomial,
               data = cnty_clus_offices)
...
```{r}
cluster_age <- glm(deported ~ criminality+ cluster +age:cluster+ gender + age + latin_cit, 
               family = binomial,
               data = cnty_clus_offices)
...
```{r}
cluster_latin_cit <- glm(deported ~ criminality + cluster +latin_cit:cluster+ gender + age + latin_cit, 
               family = binomial,
               data = cnty_clus_offices)
...
cluster_office_df <- cluster_office_df|>
  rename("clstr_lat_fixed" = "lat_fixed")
```
...
cluster_office_df_transf <- cluster_office_df |>
  mutate(across(
    .cols = c(cluster_crim_effect, aor_crim_effect, cluster_lat_effect, cluster_age_effect,
              clstr_crim_fixed, aor_crim_fixed, clstr_age_fixed, clstr_lat_fixed),
    .fns = ~ (exp(.) - 1) * 100,
    .names = "{.col}_pctdiff"
  )) 
...
```{r}
# ols first
ols_fit <- lm(prop_deported ~ prop_latin_cit + prop_crim_or_pending + prop_female + num_encounters,
             data = office_agg_sp)
